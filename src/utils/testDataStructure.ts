...
export function testPrimeSlotSystem() {
  ...
  if (detachment) {
    ...
    detachment.slots.forEach(slot => {
      ...
      console.log(
        `  ${slot.count}x ${role?.name} ${slot.isPrime ? '(Prime)' : '(Non-Prime)'}`
      );
    });
  }
}
...
export function testAlliedDetachmentAvailability() {
  ...
  const testArmyList = ArmyListStorage.createNewArmyList(
    'dark-angels',
    2000,
    'Loyalist'
  );
  ...
  console.log(
    ...
    availableDetachments.map(d => d.name)
  );

  // Check specifically for Allied detachments
  const alliedDetachments = availableDetachments.filter(
    d => d.type === 'Allied'
  );
  console.log(
    'Available Allied detachments:',
    alliedDetachments.map(d => d.name)
  );

  // Test individual availability check
  if (alliedDetachment) {
    const isAvailable = DataLoader.isDetachmentAvailable(
      alliedDetachment,
      testArmyList
    );
