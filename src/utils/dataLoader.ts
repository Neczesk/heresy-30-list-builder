...
export class DataLoader {
  ...
  static getSpecialRuleById(id: string): SpecialRule | undefined {
    return ((specialRulesData as any).specialRules as SpecialRule[]).find(rule => rule.id === id);
  }

  static getSpecialRulesByIds(ids: string[]): SpecialRule[] {
    return ((specialRulesData as any).specialRules as SpecialRule[]).filter(rule => ids.includes(rule.id));
  }
  ...
  static getModelById(id: string): Model | undefined {
    return ((modelsData as any).models as Model[]).find(model => model.id === id);
  }

  static getModelsByType(type: string): Model[] {
    return ((modelsData as any).models as Model[]).filter(model => model.type === type);
  }

  static getModelsBySubType(subType: string): Model[] {
    return ((modelsData as any).models as Model[]).filter(model => model.subType === subType);
  }

  static getModelsByTypeAndSubType(type: string, subType: string): Model[] {
    return ((modelsData as any).models as Model[]).filter(model => model.type === type && model.subType === subType);
  }
  ...
  static getWeaponById(id: string): Weapon | undefined {
    return ((weaponsData as any).weapons as Weapon[]).find(weapon => weapon.id === id);
  }

  static getRangedWeapons(): RangedWeapon[] {
    return ((weaponsData as any).weapons as Weapon[]).filter(weapon => weapon.type === 'ranged') as RangedWeapon[];
  }

  static getMeleeWeapons(): MeleeWeapon[] {
    return ((weaponsData as any).weapons as Weapon[]).filter(weapon => weapon.type === 'melee') as MeleeWeapon[];
  }

  static getWeaponsByType(type: 'ranged' | 'melee'): Weapon[] {
    return ((weaponsData as any).weapons as Weapon[]).filter(weapon => weapon.type === type);
  }

  static getWeaponsByIds(ids: string[]): Weapon[] {
    return ((weaponsData as any).weapons as Weapon[]).filter(weapon => ids.includes(weapon.id));
  }
  ...
  static getRangedWeaponsForModel(modelId: string): RangedWeapon[] {
    return this.getWeaponsForModel(modelId).filter(weapon => this.isRangedWeapon(weapon)) as RangedWeapon[];
  }

  static getMeleeWeaponsForModel(modelId: string): MeleeWeapon[] {
    return this.getWeaponsForModel(modelId).filter(weapon => this.isMeleeWeapon(weapon)) as MeleeWeapon[];
  }
  ...
  static getFactionById(id: string): Faction | undefined {
    return ((factionsData as any).factions as Faction[]).find(faction => faction.id === id);
  }

  static getFactionsByType(type: string): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => faction.type === type);
  }

  static getFactionsByAllegiance(allegiance: Allegiance): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => 
      faction.allegiance === allegiance || faction.allegiance === 'Universal'
    );
  }

  static getFactionsByAllegianceAndType(allegiance: Allegiance, type: string): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => 
      (faction.allegiance === allegiance || faction.allegiance === 'Universal') && faction.type === type
    );
  }

  static getMainFactions(): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => faction.isMainFaction);
  }

  static getMainFactionsByAllegiance(allegiance: Allegiance): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => 
      faction.isMainFaction && (faction.allegiance === allegiance || faction.allegiance === 'Universal')
    );
  }

  static getSubFactions(parentFactionId: string): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => faction.parentFaction === parentFactionId);
  }

  static getSubFactionsByAllegiance(parentFactionId: string, allegiance: Allegiance): Faction[] {
    return ((factionsData as any).factions as Faction[]).filter(faction => 
      faction.parentFaction === parentFactionId && (faction.allegiance === allegiance || faction.allegiance === 'Universal')
    );
  }
  ...
  static getRiteOfWarById(id: string): RiteOfWar | undefined {
    return ((ritesOfWarData as any).ritesOfWar as RiteOfWar[]).find(rite => rite.id === id);
  }

  static getRitesOfWarByFaction(factionId: string): RiteOfWar[] {
    return ((ritesOfWarData as any).ritesOfWar as RiteOfWar[]).filter(rite => rite.faction === factionId);
  }
  ...
  static getBattlefieldRoleById(id: string): BattlefieldRole | undefined {
    return ((battlefieldRolesData as any).battlefieldRoles as BattlefieldRole[]).find(role => role.id === id);
  }
  ...
  static getDetachmentById(id: string): Detachment | undefined {
    return ((detachmentsData as any).detachments as Detachment[]).find(detachment => detachment.id === id);
  }

  static getDetachmentsByType(type: string): Detachment[] {
    return ((detachmentsData as any).detachments as Detachment[]).filter(detachment => detachment.type === type);
  }
        ...
        case 'faction-must-match':
          if (restriction.value === 'primary-faction') {
            ...
            if (primaryDetachment) {
              const primaryDet = this.getDetachmentById(primaryDetachment.detachmentId);
  ...
  private static checkDetachmentTriggers(detachment: Detachment, armyList: ArmyList): boolean {
    ...
    for (const trigger of detachment.triggers) {
      switch (trigger.type) {
        ...
        case 'specific-unit':
          return this.hasSpecificUnitInSlot(armyList, trigger.requiredUnitId!, trigger.requiredSlotType!);
        default:
          return false;
      }
    }
  ...
  private static hasSpecificUnitInSlot(armyList: ArmyList, requiredUnitId: string, requiredSlotType: 'Command' | 'High Command'): boolean {
    ...
    for (const armyDetachment of armyList.detachments) {
      for (const unit of armyDetachment.units) {
        ...
        if (unitData.battlefieldRole === roleData.type && unit.unitId === requiredUnitId) {
          return true;
        }
      }
    }
  ...
  static getUnitsByFaction(factionId: string): Unit[] {
    return ((unitsData as any).units as Unit[]).filter(unit => unit.faction === factionId);
  }

  static getUnitsByAllegiance(allegiance: Allegiance): Unit[] {
    return ((unitsData as any).units as Unit[]).filter(unit => 
      unit.allegiance === allegiance || unit.allegiance === 'Universal'
    );
  }

  static getUnitsByFactionAndAllegiance(factionId: string, allegiance: Allegiance): Unit[] {
    return ((unitsData as any).units as Unit[]).filter(unit => 
      unit.faction === factionId && (unit.allegiance === allegiance || unit.allegiance === 'Universal')
    );
  }

  static getUnitsByRole(roleId: string): Unit[] {
    return ((unitsData as any).units as Unit[]).filter(unit => unit.battlefieldRole === roleId);
  }

  static getUnitsByRoleAndAllegiance(roleId: string, allegiance: Allegiance): Unit[] {
    return ((unitsData as any).units as Unit[]).filter(unit => 
      unit.battlefieldRole === roleId && (unit.allegiance === allegiance || unit.allegiance === 'Universal')
    );
  }
  ...
  static getUpgradeById(id: string): UnitUpgrade | undefined {
    return ((upgradesData as any).upgrades as UnitUpgrade[]).find(upgrade => upgrade.id === id);
  }



  static getUpgradeOptionById(upgradeId: string, optionId: string): UpgradeOption | undefined {
  ...
  static getModelsForUnitInstance(unitId: string, armyUnit: ArmyUnit): { model: Model; count: number }[] {
    const baseUnit = this.getUnitById(unitId);
    if (!baseUnit) return [];
  ...
  static calculateWeaponCounts(unitId: string, armyUnit: ArmyUnit): { [key: string]: { weaponId: string; count: number; mount?: string } } {
    const baseUnit = this.getUnitById(unitId);
    if (!baseUnit) return {};
    ...
    console.log('Model instance weapon changes:', armyUnit.modelInstanceWeaponChanges);
    ...
    modelComposition.forEach(armyModel => {
      ...
      if (baseModel) {
        ...
        for (let instanceIndex = 0; instanceIndex < armyModel.count; instanceIndex++) {
          console.log(`\n  Processing instance ${instanceIndex}:`);
          
          // Start with base weapons for this instance
          let instanceWeapons: Array<{ id: string; mount?: string; count?: number }> = [];
          
          // Handle new weapon structure with mount locations
          if (Array.isArray(baseModel.weapons) && baseModel.weapons.length > 0 && typeof baseModel.weapons[0] === 'object' && 'id' in baseModel.weapons[0]) {
            // New structure: Array of { id: string, mount?: string, count?: number }
            instanceWeapons = [...(baseModel.weapons as Array<{ id: string; mount?: string; count?: number }>)];
          } else {
            // Old structure: Array of strings
            instanceWeapons = (baseModel.weapons as unknown as string[]).map(id => ({ id }));
          }
          
          console.log(`  Initial weapons for instance ${instanceIndex}:`, instanceWeapons);
          ...
          if (armyUnit.modelInstanceWeaponChanges && 
              ...
              armyUnit.modelInstanceWeaponChanges[armyModel.modelId][instanceIndex]) {
            
            const changes = armyUnit.modelInstanceWeaponChanges[armyModel.modelId][instanceIndex];
            console.log(`  Weapon changes for instance ${instanceIndex}:`, changes);
            ...
            for (let i = 0; i < Math.max(removedWeapons.length, addedWeapons.length); i++) {
              if (i < removedWeapons.length) {
                const weaponToRemove = removedWeapons[i];
                const weaponIdToRemove = typeof weaponToRemove === 'string' ? weaponToRemove : weaponToRemove.id;
                const weaponMountToRemove = typeof weaponToRemove === 'string' ? undefined : weaponToRemove.mount;
                ...
                if (weaponMountToRemove) {
                  ...
                  instanceWeapons = instanceWeapons.filter(weapon => 
                    !(weapon.id === weaponIdToRemove && weapon.mount === weaponMountToRemove)
                  );
                  console.log(`    Removing ${weaponIdToRemove} with mount ${weaponMountToRemove}`);
                } else {
                  // Remove all instances of this weapon (fallback for old system)
                  instanceWeapons = instanceWeapons.filter(weapon => weapon.id !== weaponIdToRemove);
              ...
              if (i < addedWeapons.length) {
                ...
                console.log(`    Adding ${typeof weaponToAdd === 'string' ? weaponToAdd : weaponToAdd.id}`);
              }
            }
          } else {
          ...
          console.log(`  Final weapons for instance ${instanceIndex}:`, instanceWeapons);
        }
      }
    });
  ...
  static getPrimeAdvantages(): PrimeAdvantageDefinition[] {
    return (primeAdvantagesData as any).primeAdvantages as PrimeAdvantageDefinition[];
  }

  static getPrimeAdvantageById(id: string): PrimeAdvantageDefinition | undefined {
    return ((primeAdvantagesData as any).primeAdvantages as PrimeAdvantageDefinition[]).find(advantage => advantage.id === id);
  }

  // Utility methods
  static calculateUnitPoints(unit: Unit, size: number, upgrades: any[] = []): number {
